...
tooltip.id = "tooltip"; // Assign an id to the tooltip
document.body.appendChild(tooltip);


/* javascript for the overlay feature appearing and disappearing / resetting the display to none */
window.addEventListener("load", function() {
    setTimeout(
        function open() {
            document.querySelector(".popupCardAboutMe").style.display = "grid";
            document.querySelector("#overlay").style.display = "block";
            document.querySelector(".popupCardAboutMe").style.opacity = "1";
...
document.addEventListener("DOMContentLoaded", function() {
    document.querySelector("#closeButtonPopup").addEventListener("click", function() {
        document.querySelector(".popupCardAboutMe").style.opacity = "0";
...
document.querySelector(".popupCardAboutMe").addEventListener("transitionend", function() {
...
/* Adding in the function to be able to press about and see the pop up again */

document.querySelector("#aboutPopup").addEventListener("click", function() {
    document.querySelector(".popupCardAboutMe").style.display = "grid";
    document.querySelector("#overlay").style.display = "block";
    document.querySelector(".popupCardAboutMe").style.opacity = "1";
    document.querySelector("#overlay").style.opacity = "1";
...
// Image sets
const imageSets = {
    set1: ['renderPersonal.png', 'uberGame.png', 'DaveCourt.png', 'cyberFeminism.png', 'speculativeFuture.png', 'draining.png', 'ThreeDMe.png', 'paradiseClub.png', 'mediaPlayer.png', 'contactMe.png'],
    set2: ['cyberFeminism.png', 'speculativeFuture.png', 'draining.png'],
    set3: ['renderPersonal.png', 'uberGame.png', 'ThreeDMe.png', 'paradiseClub.png', 'mediaPlayer.png', 'contactMe.png']
    // Add more sets as needed
};

// Initial set of images
let imageFiles = [...imageSets.set1];
...
const tooltipTexts = {
    "contactMe.png": "Tooltip text for contactMe",
    "DaveCourt.png": "Tooltip text for DaveCourt",
    // Add the rest of your mappings here
}

let testString = "insta.png"

function updateImageFiles(imageSets) {
    imageFiles = [...imageSets.set1]; // Update imageFiles with the new set
    effect.recreateParticles();
    console.log("Updated image files:", imageFiles); // Optional: for debugging
}

function updateImageFiles2(imageSets) {
    imageFiles = [...imageSets.set2]; // Update imageFiles with the new set
    effect.recreateParticles();
    console.log("Updated image files:", imageFiles); // Optional: for debugging
}

function updateImageFiles3(imageSets) {
    imageFiles = [...imageSets.set3]; // Update imageFiles with the new set
    effect.recreateParticles();
    console.log("Updated image files:", imageFiles); // Optional: for debugging
}
...
class Particle {
    constructor(effect) {
        ...
        if (imageFiles.length === 0) {
            // No more images left, stop creating particles
            throw new Error("No more images left");
        }
            // Pop an image from the array
    ...
    canvas.addEventListener('mousemove', e => {
        ...
        if (hoveredParticle) {
            const xOffset = 50;
            tooltip.textContent = tooltipTexts[hoveredParticle.imageSrc]; // Use imageSrc to get the tooltip text
            tooltip.style.left = e.pageX + xOffset + "px";
            tooltip.style.top = e.pageY - xOffset + "px";
            ...
            for (const particle of this.effect.particles) {
                ...
                if (particle !== this) {
                    ...
                    if (distance < (this.size + particle.size) / 2) {
                        ...
                        if (!this.collided) {
                            this.vx *= 0.9999; // Horizontal  damping
                            this.vy *= 0.9999; // Vertical  damping
    ...
    createParticles() {
        for (let i = 0; i < this.numberOfParticles; i++) {
            try {
                ...
                // No more images left, stop creating particles
                console.log("Stopped creating particles, no more images left.");
                break;
            }
        }
    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    /* Extending Heritage */
    const popupExtendingHeritage = document.getElementById("ExtendingHeritageDiv");
    const closeButtonExtendingHeritage = document.getElementById("closeButtonExtendingHeritage");

    closeButtonExtendingHeritage.addEventListener("click", () => {
        toggleDisplay(popupExtendingHeritage, closeButtonExtendingHeritage);
    });
    ...
    /* BrewFinder */
    const popupCardBrewFinder = document.getElementById("brewFinderDiv");
    const closeButtonBrewFinder = document.getElementById("closeButtonBrewFinder");
